...
from celoetl_airflow.bigquery_utils import submit_bigquery_job
...
def build_load_dag(
    ...
    def add_enrich_tasks(task, time_partitioning_field='block_timestamp', dependencies=None, always_load_all_partitions=False):
        def enrich_task(ds, **kwargs):
            ...
            template_context['params']['tx_join_field'] = 'transaction_index'
            template_context['params']['trace_join_field'] = 'transaction_index'
    ...
    def add_deduplicate_tasks(task, dependencies=None):
        sql_path = os.path.join(dags_folder, 'resources/stages/deduplicate/sqls/{task}.sql'.format(task=task))
        sql = read_file(sql_path)
        deduplicate_task = BigQueryOperator(
            task_id='deduplicate_{task}'.format(task=task),
            bql=sql,
            params=environment,
            use_legacy_sql=False,
            dag=dag)
    ...
    if load_traces:
        # verify_traces_blocks_count_task = add_verify_tasks('traces_blocks_count',
        #                                                    [enrich_blocks_task, enrich_traces_task])
    ...
    if load_traces and load_contracts:
    ...
    if notification_emails and len(notification_emails) > 0:
        send_email_task = EmailOperator(
            ...
            subject='Celo ETL Airflow Load DAG Succeeded',
            html_content='Celo ETL Airflow Load DAG Succeeded - {}'.format(chain),
            dag=dag
        )

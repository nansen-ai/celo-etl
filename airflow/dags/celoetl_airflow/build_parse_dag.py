...
from celoetl_airflow.bigquery_utils import create_view, share_dataset_all_users_read
from celoetl_airflow.common import read_json_file, read_file
from celoetl_airflow.parse.parse_logic import ref_regex, parse, create_dataset
...
def build_parse_dag(
    ...
    PARTITION_DAG_ID = f'{chain_name}_partition_dag'
    ...
    # This prevents failing all dags as they are constructed in a loop in celo_parse_dag.py
    ...
    wait_for_partition_dag_task = ExternalTaskSensor(
        task_id='wait_for_partition_dag',
    ...
    for json_file in json_files:
        ...
        wait_for_partition_dag_task >> task
    ...
    full_dataset_name = f'{chain_name}_' + dataset_name
    ...
    if notification_emails and len(notification_emails) > 0 and send_success_email:
        send_email_task = EmailOperator(
            ...
            subject=f'{chain_name} ETL Airflow Parse DAG Succeeded',
            html_content=f'{chain_name} ETL Airflow Parse DAG Succeeded for {dag_id}',
            dag=dag
        )
